...
public class Rational implements Comparable<Rational> {
    ...
    private static long gcd(long m, long n) {
        if (m < 0) { m = -m; }
        if (n < 0) { n = -n; }
        if (0 == n) { return m; } else { return gcd(n, m % n); }
    }

    /**
     * //https://zh.wikipedia.org/wiki/%E8%BC%BE%E8%BD%89%E7%9B%B8%E9%99%A4%E6%B3%95 循环（非递归）求解最大公约数
     *
     * @param a 除数
     * @param b 余数
     * @return
     */
    private static long nonRecursiveGCD(long a, long b) {
        while (b != 0) {
            long t = b;
            b = a % b;
            a = t;
    ...
    public String toString() {
        if (den == 1) { return num + ""; } else { return num + "/" + den; }
    }
    ...
    // return |this|
    public Rational abs() {
        if (num >= 0) { return this; } else { return negate(); }
    }
