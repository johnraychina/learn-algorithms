...
public class RedBlackLiteBST<Key extends Comparable<Key>, Value> {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < keys.length; i++) { st.put(keys[i], i); }
    ...
    public Value get(Node x, Key key) {
        while (x != null) {
            ...
            if (cmp < 0) { x = x.left; } else if (cmp > 0) { x = x.right; } else { return x.val; }
        }
        return null;
    }
    ...
    private Node insert(Node h, Key key, Value val) {
        ...
        if (cmp < 0) { h.left = insert(h.left, key, val); } else if (cmp > 0) {
